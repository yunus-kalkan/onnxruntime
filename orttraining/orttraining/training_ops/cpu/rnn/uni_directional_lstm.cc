...
void UniDirectionalLstmTraining<T>::Compute(const gsl::span<const T>& inputs_arg,
                                            ...
                                            gsl::span<T>& outputs, gsl::span<T>& all_cell_states,
  ...
  auto sequences_calculator = [&](int seq_start, onnxruntime::concurrency::ThreadPool* ttp) {
    ...
    for (int step = 0; step < max_sequence_length; step++) {
      ...
      /*
      Changes for training in this block.
      */
      // namespace training
      {
        gsl::span<const T> src = batched_internal_memory_prev_;
        gsl::span<T> dst = all_cell_states.subspan(step * output_step_length, output_step_length);
        gsl::copy(src, dst);
      }
...
template <typename T>
void UniDirectionalLstmTraining<T>::ComputeGradient() {
}
...
template void UniDirectionalLstmTraining<float>::Compute<float>(
    ...
    gsl::span<float>& outputs, gsl::span<float>& all_cell_states,
