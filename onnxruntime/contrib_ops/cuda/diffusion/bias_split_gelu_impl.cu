...
#pragma unroll
  for (int32_t i = 0; i < HHS / TPB; ++i) {
    const T left = input[index_input] + bias[index_bias];
    const T right = input[index_input + HHS] + bias[index_bias + HHS];
    float value_left = float(left);
    float value_right = float(right);
