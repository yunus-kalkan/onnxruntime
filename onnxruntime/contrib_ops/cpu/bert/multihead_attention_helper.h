...
Status CheckInputs(const T* query,
  ...
  //   query            (Q)       : (B, S, D)
  //   key              (K)       : (B, L, D) or (B, L, N, 2, H)
  //   value            (V)       : (B, L, D_v) or None
  ...
  if (bias != nullptr) {
    ...
    // Currently, packed KV and no bias is enabled to use with fused trt cross attentin kernel only.
    // This constraint can be removed later.
    if (value != nullptr) {
      return ORT_MAKE_STATUS(ONNXRUNTIME, INVALID_ARGUMENT, "'value' and 'bias' shall both be none for packed kv. ");
    }
  }
