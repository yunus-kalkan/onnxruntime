...
MlasQLinearSafePaddingElementCount(size_t ElementSize, size_t ElementCount)
{
    if (!(ElementSize == 1 || ElementSize == 2 || ElementSize == 4 || ElementSize == 8 ||
          ElementSize == 16)) {
        MLAS_THROW_EX(
            std::invalid_argument("ElementSize must be power of 2 and less or equal than 16!"));
    }
...
CheckQLinearGlobalAveragePoolScaleAndSize(float ScaleInput, float ScaleOutput, size_t ImageSize)
{
    ...
    if (scale < 0x1.0p-32f || scale >= 256.0f) {
        ...
        MLAS_THROW_EX(
            std::invalid_argument("QLinearGlobalAveragePool parameter out of computation range!"));
    }
    return scale;
}
